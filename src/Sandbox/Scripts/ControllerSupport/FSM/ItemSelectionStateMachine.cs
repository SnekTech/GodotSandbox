...
abstract class ItemSelectionState(ItemSelectionStateMachine stateMachine) : IState
{
    ...
    internal virtual Task HandleInputAsync(DirectionSelectInput input, CancellationToken ct = default) => Task.CompletedTask;
 ...
 class ItemSelectionStateMachine : StateMachineV2<ItemSelectionState>
 {
    ...
    internal Task HandleInputAsync(DirectionSelectInput input, CancellationToken ct = default) =>
        CurrentState?.HandleInputAsync(input, ct) ?? Task.CompletedTask;
